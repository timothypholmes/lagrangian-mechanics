...
class double_pendulum:
    ...
    def derivative(self, y, t, L1, L2, m1, m2, g):
        ...
        dthetadot_dt1 = ((g*(np.sin(theta2)*np.cos(theta1 - theta2) - ((1+(m1+m2))*np.sin(theta1)) 
        - (L2*thetadot2**2+L1*thetadot1*np.cos(theta1-theta2)*np.sin(theta1-theta2))/(L1*((1+(m1+m2))-np.cos(theta1-theta2)**2)))))


        dthetadot_dt2 = ((g*(1+(m1+m2)) * (np.sin(theta1)*np.cos(theta1-theta2)-np.sin(theta2)) 
        + ((1+(m1+m2))*L1*thetadot1**2+L2*thetadot2**2*np.cos(theta1-theta2)*np.sin(theta1-theta2))) / (L2*((1+(m1+m2)-np.cos(theta1-theta2)))))
